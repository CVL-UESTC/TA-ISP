...
class DetDataPreprocessor(ImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        # print(f"归一化前:{data['inputs'][0].max()}")
        # print(f"归一化前shape:{data['inputs'][0].shape}")
        data = super().forward(data=data, training=training)
        # print(f"归一化后均值0:{data['inputs'][0][0,...].mean()}")#这一步归一化
        # print(f"归一化后均值1:{data['inputs'][0][1,...].mean()}")
        # print(f"归一化后均值2:{data['inputs'][0][2,...].mean()}")
        # print(f"归一化后均值3:{data['inputs'][0][3,...].mean()}")
        # print(f"归一化后shape:{data['inputs'][0].shape}")
